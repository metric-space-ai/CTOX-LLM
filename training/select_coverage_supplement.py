...
def candidate_coverage(
    ...
    if requirements["domain_primary"][primary] > 0:
    ...
    while any(value > 0 for counter in requirements.values() for value in counter.values()):
        ...
        for record, tag, confidence in eligible:
            ...
            if coverage:
                ...
                sample_id = str(record["id"])
                ranked.append(
                    (-score, -confidence, token_counts[sample_id], sample_id, record, tag, coverage)
                )
        if not ranked:
            unresolved = {
...
def main() -> None:
    ...
    try:
        ...
        selected, selection = select_joint_supplement(
            baseline,
            baseline_tags,
            candidates,
            candidate_tags,
        ...
        combined_tags.update({str(record["id"]): candidate_tags[str(record["id"])] for record in selected})

...
| Runtime | Checksummed v1/v2 container, corrected memory planner, graph ownership plan, stable lifecycle/wire contracts, signed/rehashed multilingual MTP draft-vocabulary contract, two-phase target+MTP verification/commit composition, fail-closed greedy MTP verification, and bring-up server that deliberately returns `engine_not_ready` | Production `ModelExecutor`, tokenizer/template binding, probability-correct non-greedy MTP, optimized streaming prefill/decode, and full-artifact unload evidence |

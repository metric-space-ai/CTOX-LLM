...
| CUDA | Pinned upstream reference sources, SM86 ABI, direct Driver API runtime, resident pure/mixed embedding plus a 505-projection/262-activation mmap-backed target+MTP graph, all 48 linear token-mixer parameter/state groups, context-sized Q2/Q4 KV for 16 target plus one MTP full-attention state, all 134 target/MTP norm operators, fused residual/RMSNorm, SwiGLU/A8/down and attention-gate/A8/output paths, exact prepared-resource bindings for all 645 frozen decode steps, compiled target-token, one-layer MTP draft, and target-verification device chains, one transactional commit barrier per complete target/MTP step, and the bounded FP16/retained-KV checkpoint/restore primitive required by replay-on-reject | Pass the queued target/MTP hardware-logit, barrier-count, and bit-exact replay verifier; bind chained MTP4 partial-prefix replay and sampling into `ModelExecutor`; finish production MMQ/prefill; run the 128K load/numerical verifiers, controlled roofline sweep, and full-model golden/unload run |
| Metal | Direct MSL compilation/dispatch, reusable resident per-op buffers, Q2/Q4 same-device oracle comparison, shape-local simdgroup tuning, and a verifier benchmark CLI on Apple M5 | Zero-copy artifact ownership, embedding/token-mixer kernels, full graph, stable controlled roofline/reference evidence, and promotion |
...
| Runtime | Checksummed v1/v2 container, corrected memory planner, graph ownership plan, stable lifecycle/wire contracts, pinned pure-Rust tokenizer plus text/reasoning/tool template, signed/rehashed multilingual MTP draft-vocabulary contract, two-phase target+MTP verification/commit composition, fail-closed greedy MTP verification, concurrent/cancellable token-ID `EngineServer`, unbuffered multilingual Responses/tool frontend, and bring-up binary that deliberately returns `engine_not_ready` | Production `ModelExecutor`, promoted-binary binding, probability-correct non-greedy MTP, optimized streaming prefill/decode, and full-artifact unload evidence |

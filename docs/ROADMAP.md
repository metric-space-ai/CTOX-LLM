...
| Metal | Direct MSL compilation/dispatch, no-copy mmap artifact ownership, recovered embedding/projection/token-mixer candidates, Q2/Q4 same-device oracle comparison, shape-local simdgroup tuning, a frozen 645-step decode schedule, exact binding of all 505 projection/262 activation resources, one allocated 1,173,760-byte alias-safe shared decode-activation arena, real buffer/offset views for every read/write of all 645 steps, single-use device-only target-hidden plus FP16 convolution/GatedDelta checkpoints, bounded append-only paged-KV rollback without arena duplication, one fail-closed atomic transaction across the final target hidden plus all 17 attention and 48 paired linear-state owners, and an RAII decode attempt that couples cursor completion to rollback/commit | Complete target+MTP executor, per-step kernel encoder dispatch and prefill arena, removal of verifier CPU KV mirror, allocator high-watermark/unload proof, stable controlled roofline/reference evidence, full-model golden run, and promotion |
...
first exact decode chain. Frozen steps 0-6 dispatch embedding, layer-0 RMSNorm,
...
direct-weight gated RMSNorm in place to `AttentionOutput`, with one command
encoder/wait. This is verified on Apple Silicon but is not backend promotion:
the remaining 638 steps, complete

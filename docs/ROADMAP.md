...
| Metal | Direct MSL compilation/dispatch, no-copy mmap artifact ownership, recovered embedding/projection/token-mixer candidates, Q2/Q4 same-device oracle comparison, shape-local simdgroup tuning, a frozen 645-step decode schedule, exact binding of all 505 projection/262 activation resources, and a 1,173,760-byte alias-safe shared decode-activation arena plan | Complete target+MTP executor, actual decode/prefill arena allocation and view binding, transactional state, allocator/unload proof, stable controlled roofline/reference evidence, full-model golden run, and promotion |

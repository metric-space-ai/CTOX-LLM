...
| CUDA | Pinned upstream reference sources, SM86 ABI, direct Driver API verifier runtime, a complete 505-projection/262-activation mmap-backed target+MTP load graph, FP16 token-mixer candidates, fused residual/RMSNorm, SwiGLU/A8/down and attention-gate/A8/output paths, a frozen 645-step decode schedule, and device-packed paged Q2/Q4 GQA | Add embedding and float token-mixer ownership, bind every schedule step, finish production MMQ/prefill/sampling, run queued same-device load/numerical verifiers, controlled roofline sweep, and full-model golden/unload run |

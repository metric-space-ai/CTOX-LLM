...
| CUDA | Pinned upstream reference sources, SM86 ABI, direct Driver API verifier runtime, resident Q2/Q4/A8 projection candidates, FP16 GatedDelta/causal-conv/norm/RoPE candidates, fused head-wise query/gate deinterleave+Q-norm+RoPE, and device-packed paged Q2/Q4 GQA with a context-bound Q/K/V/output device-view contract and one-pass online softmax | Add residual/FFN fusion and connect the device views through the full decoder, finish production MMQ/prefill/sampling paths, run the queued same-device verifiers, controlled roofline sweep, and full-model golden/unload run |

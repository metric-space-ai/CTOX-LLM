...
| CUDA | Pinned upstream reference sources, SM86 ABI, direct Driver API verifier runtime, resident mmap-backed Q2/Q4/A8 projections, FP16 GatedDelta/causal-conv/norm/RoPE candidates, fused residual/RMSNorm, SwiGLU/A8/down and attention-gate/A8/output paths, a frozen 645-step target+MTP decode schedule, and device-packed paged Q2/Q4 GQA | Bind the complete schedule to artifact-prepared operators, finish production MMQ/prefill/sampling paths, run the queued same-device verifiers, controlled roofline sweep, and full-model golden/unload run |

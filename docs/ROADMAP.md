...
| Metal | Direct MSL compilation/dispatch, no-copy mmap artifact ownership, recovered embedding/projection/token-mixer candidates, Q2/Q4 same-device oracle comparison, shape-local simdgroup tuning, a frozen 645-step decode schedule, exact binding of all 505 projection/262 activation resources, one allocated 1,173,760-byte alias-safe shared decode-activation arena with exact offset views, and single-use device-only target-hidden plus FP16 convolution/GatedDelta checkpoint/restore primitives | Complete target+MTP executor, per-step decode view binding and prefill arena, transactional paged KV and graph-wide atomic orchestration, allocator high-watermark/unload proof, stable controlled roofline/reference evidence, full-model golden run, and promotion |

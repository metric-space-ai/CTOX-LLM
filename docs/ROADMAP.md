...
| CUDA | Pinned upstream reference sources, SM86 ABI, direct Driver API verifier runtime, resident Q2/Q4 fused-matvec candidate, and first GPU3 numerical/per-op benchmark | Upstream-derived dp4a/MMQ path, remaining graph kernels, controlled roofline sweep, allocator ownership, and full-model golden run |

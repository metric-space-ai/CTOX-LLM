...
| Runtime | Checksummed v1/v2 container, corrected memory planner, graph ownership plan, stable lifecycle/wire contracts, pinned pure-Rust tokenizer plus text/reasoning/tool template, signed/rehashed multilingual MTP draft-vocabulary contract, two-phase target+MTP verification/commit composition, fail-closed greedy MTP verification, concurrent/cancellable token-ID `EngineServer` adapter, and bring-up binary that deliberately returns `engine_not_ready` | Production `ModelExecutor`, Responses text frontend and promoted-binary binding, probability-correct non-greedy MTP, optimized streaming prefill/decode, and full-artifact unload evidence |

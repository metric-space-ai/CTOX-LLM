...
| CUDA | Pinned upstream reference sources, SM86 ABI, direct Driver API runtime, a 505-projection/262-activation mmap-backed target+MTP graph, all 48 linear token-mixer parameter/state groups, context-sized Q2/Q4 KV for 16 target plus one MTP full-attention state, fused residual/RMSNorm, SwiGLU/A8/down and attention-gate/A8/output paths, and a frozen 645-step decode schedule | Add embedding and remaining decoder/MTP norm ownership, bind every schedule step, finish production MMQ/prefill/sampling, run queued same-device 128K load/numerical verifiers, controlled roofline sweep, and full-model golden/unload run |

...
fn main() -> anyhow::Result<()> {
    ...
    for token in 0..args.tokens {
        ...
        prepared.write_step(&query, &key, &value, &log_decay, &beta)?;
        let actual = runtime.dispatch_gated_delta_f16(&mut prepared)?;
    ...
    anyhow::ensure!(
        observed_reclaimed_bytes >= persistent_state_bytes + transient_buffer_bytes,
    ...
    println!(
        ...
        serde_json::to_string_pretty(&Report {
            format: "ctox.cuda-sm86-gated-delta-f16-verifier.v1",
            status: "pass",
            ...
            reset_zero_state_verified,
            note: "Verifier-only candidate; no CPU fallback and not promoted into the production CUDA ABI.",
        })?
    );
    Ok(())
}
